...
public class Pccp001 {

	public int solution(int[] bandage, int health, int[][] attacks) {
        int answer = 0;
        int healthMax = health;
        int healStack = 0;
        int attackTurnIndex = 0;
        int attackTurnSec = 0;
        ...
        for(int i = 0 ; i < attacks[attacks.length-1][0] +1; i++) {
			...
			if(health > 0) {
				if(health <= healthMax) {
					for(int k = attackTurnIndex ; k<attacks.length; k++) {
						if(i == attacks[k][0]) {//공격받은 턴 계산
							attackTurnIndex = k;
							attackTurnSec = i;
							continue;
						}
					}
					if(i == attackTurnSec) {//공격
						healStack = 0;
						health = health - attacks[attackTurnIndex][1];
	...
	public static void main(String[] args) {
		...
		//int health = 0;
		int health = 30;
		//int attacks[][] = new int[2][100];
		int attacks[][] = {{2,10},{9,15},{10,5},{11,5}};
		
		//System.out.println(attacks[1][0]);
		
		Pccp001 asd = new Pccp001();
		int asdf = asd.solution(bandage, health, attacks);
		
		System.out.println("최종 : "+asdf);
	}
